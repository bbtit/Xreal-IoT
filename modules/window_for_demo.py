'''
Code for shooting concept video of proposed system
'''
...
class WindowCanvasManager:
    def __init__(self, bg_color="black"):
        ...
        # -- デモ動画用のインスタンス変数 --
        self.speaker_names = ["1人目", "2人目", "3人目", "4人目"]
    ...
    def draw_voice_angle_arc_and_text_forever(
        ...
        if input_text is not None:
            # デモ用の変更点:テキストに名前を追加し，スピーカー番号を更新
            speaker_name = self.speaker_names[self.speaker_number]
            self.speaker_number = (self.speaker_number + 1) % len(self.speaker_names)
            # スピーカーの名前を追加
            full_text = f"{speaker_name}: {input_text}"
            ...
            # テキストを更新
                ...
                self.update_text_content(self.drown_text_id, full_text)
