...
このファイルには、speaker-id に使用する非常にシンプルな TDNN モジュールが含まれています。

このモデルを置き換えるには、ハイパーパラメータファイルの `!new:` タグを変更してください。
タグを変更して、組み込みのSpeechBrainモデルか PyTorchのカスタムモジュールを含む別のファイルを参照してください。
...
class Xvector(torch.nn.Module):
    ...
    def __init__(
        ...
        TDNNは与えられた拡張係数とカーネルサイズで畳み込み層を持つ。
        ここでは、追加したいすべての畳み込み層をループする。
        この場合、活性化関数の後にバッチ正規化が使われることに注意する。
        これにより スピーカーIDの性能が少し向上する。
        """
        ...
        統計的プーリング。
        これは可変長のテンソルを固定長のテンソルに変換します。
        統計的プーリングは平均と標準偏差を返す。
        T*Fの形状をしたテンソルを受け取り(Tはフレーム数、Fは特徴量の数)、
        2Fの形状をしたテンソルを返す。
        """
